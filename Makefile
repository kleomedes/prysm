...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=prysm \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/prysmd ./cmd/prysmd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/prysmd.exe ./cmd/prysmd
...
draw-deps:
	...
	@goviz -i ./cmd/prysmd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c prysm --local -f chains.yaml
endif
...
set-testnet-configs:
	prysmd config set client chain-id localchain-1
	prysmd config set client keyring-backend test
	prysmd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | prysmd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | prysmd keys add acc1 --recover`

testnet: setup-testnet
	local-ic start testnet
